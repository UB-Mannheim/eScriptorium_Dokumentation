...
- `index.md` is the Pages landing page and mirrors `README.md` — keep both in sync when adding/renaming guides (both group guides into "Aktuelle Oberfläche" and "Legacy-Oberfläche").
...
- Docs are split by **interface generation**: legacy UI (eScriptorium pre-1.0) vs current UI (1.0+). Keep each guide's scope note (top blockquote) and its section in `README.md`/`index.md` consistent with that split.
